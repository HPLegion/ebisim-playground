...
k = eb.Target.get_ions("K", 1e-6 * 1.5e15, kT=80)
ne = eb.Target.get_gas("Ne", 1e-10, .005)
# k = eb.Target.get_gas("Ar", .5e-10, 1e-4)
dev = eb.Device.get(1, 8000, 1e-4, 0.8, 1300, 2, 0.005, n_grid=150,)
print(ne.n)
print(k.n)
...
TMAX = 10
T0 = 0.0001
res = eb.advanced_simulation(dev, k, TMAX, options=eb.ModelOptions(RADIAL_DYNAMICS=True), solver_kwargs={"dense_output":True, "max_step":1}, rates=True)
# res.plot_rate(eb.Rate.E_KIN_MEAN)
...
res.plot_temperature(ax=axs[0, 2], title=None, ylim=(10,10000), dens_threshold=0)
kr.plot_temperature(ax=axs[1, 2], title=None, ylim=(10,10000), dens_threshold=0)
...
for i, t in enumerate(np.geomspace(T0, TMAX, 600)):
    ...
    # axs[0, 1].set_title(res._param_title("Radial distribution"))
    axs[0, 1].set_title(res._param_title(""))
    plt.savefig(f"./img/K4a_{i:04}.png")
    # plt.close()
    # plt.show()
